...
**Demo Steps**:
1. Copy the following from monolith to customer-service:
...
2. Use Copilot to fix package names:
   - Select all files
   - Use Copilot Chat: "Update all package names to com.fisglobal.customer.*"
...
**Demo Steps**:
1. Create directory: `mkdir inventory-service`
2. Use Copilot Chat with this prompt:

**Copilot Chat Prompt**:
```
Create a Spring Boot microservice called inventory-service on port 8082.
It should be similar to the customer-service we just created.
Include the Product entity, ProductRepository, ProductService, and ProductController.
Use the code from the inventory package in the monolith as reference.
```

**Demo Steps**:
1. Let Copilot generate the files
...
**Demo Steps**:
1. Create directory: `mkdir order-service`
2. Use Copilot to generate basic structure (similar to previous services)
